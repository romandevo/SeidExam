...
// Generate PDF directly from JSON (always full dataset)
async function generatePDFfromJSON() {
  const { jsPDF } = window.jspdf;
  const doc = new jsPDF();

  // ✅ Fetch full dataset from endpoint
  ...
  // ✅ Return Blob instead of bloburl (Safari fix)
...
pdfButton.addEventListener('click', async () => {
  ...
  pdfViewer.src = blobUrl; // Safari/iOS will recognize PDF now
...
pdfFullscreenBtn.addEventListener('click', () => {
  pdfSheet.classList.toggle('fullscreen');
});
