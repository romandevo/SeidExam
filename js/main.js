...
const pdfButton = document.getElementById('showPDF');
const pdfViewer = document.getElementById('pdfViewer');
const dragHandle = pdfSheet.querySelector('.drag-handle');

// --- PDF generation (unchanged, just wrapped in try/catch) ---
async function generatePDFfromJSON() {
  ...
  data.forEach(q => {
    doc.setFontSize(12);
    doc.text(`${q.questionNumber}. ${q.questionUp}`, 10, y);
    y += lineHeight;
    doc.text(q.questionDown, 10, y);
    y += lineHeight;

    q.answers.forEach(ans => {
      const [key, value] = Object.entries(ans)[0];
...
// Open sheet then load PDF (sheet appears immediately)
pdfButton.addEventListener('click', async () => {
  pdfSheet.classList.add('show');
  try {
    const pdfBlob = await generatePDFfromJSON();
    ...
    // keep the sheet open; viewer will remain blank
  }
});

// Close
pdfCloseBtn.addEventListener('click', () => {
  pdfSheet.classList.remove('show', 'fullscreen');
  pdfSheet.style.height = ''; // clear inline drag height
  pdfViewer.src = ''; // cleanup
});

// ---------- Drag-to-Fullscreen (Pointer Events) ----------
let startY = 0;
let startHeight = 0;
...
// Use a progress threshold: if dragged 25% of the way toward fullscreen, snap to 100%
function snapThresholdPx() {
  const minH = minHeightPx();
  const maxH = maxHeightPx();
  return Math.round(minH + (maxH - minH) * 0.25);
}

function onPointerDown(e) {
  // Only when sheet is visible
  if (!pdfSheet.classList.contains('show')) return;

  isDragging = true;
  pdfSheet.style.transition = 'none'; // disable height transition during live drag

  startY = e.clientY ?? (e.touches && e.touches[0].clientY) ?? 0;

  // Current effective height (if inline set, use it; else compute from rect)
  const rect = pdfSheet.getBoundingClientRect();
  startHeight = rect.height > 0 ? rect.height : minHeightPx();
...
function onPointerMove(e) {
  ...
  const dy = startY - currentY; // dragging up => positive

  const minH = minHeightPx();
  const maxH = maxHeightPx();

  // New height between 80% and 100% of viewport
  const newHeight = clamp(startHeight + dy, minH, maxH);
  pdfSheet.style.height = `${newHeight}px`;
...
function onPointerUp(e) {
  ...
  pdfSheet.style.transition = 'height 0.2s ease, bottom 0.35s ease';

  if (currentH >= snapThresholdPx()) {
    // Snap to fullscreen
    pdfSheet.classList.add('fullscreen');
    pdfSheet.style.height = ''; // let CSS class control the height
  } else {
    // Snap back to default 80%
    pdfSheet.classList.remove('fullscreen');
    pdfSheet.style.height = ''; // let base CSS (80vh) take over
  }

  // release pointer capture if any
  if (dragHandle.releasePointerCapture && e.pointerId != null) {
    try {
